...
class AudioPlaybackProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this.minStartSamples = 1024;

    this.port.onmessage = (event) => {
      const { type, samples, minStartSamples } = event.data;
      if (type === "audio") {
        this.ringBuffer.push(samples);
        if (!this.isPlaying && this.ringBuffer.available() >= this.minStartSamples) {
          this.isPlaying = true;
        }
      } else if (type === "config") {
        this.minStartSamples = Math.max(128, minStartSamples || this.minStartSamples);
      } else if (type === "clear") {
        ...
        if (!this.isPlaying && this.ringBuffer.available() > 0) {
          this.isPlaying = true;
        }
      } else if (type === "stop") {
        this.isPlaying = false;
  ...
  process(inputs, outputs) {
    ...
    if (this.isPlaying) {
      ...
      channel.fill(0);
      if (!this.streamComplete && this.ringBuffer.available() >= this.minStartSamples) {
        this.isPlaying = true;
      }
    }
    return true;
